...
// TestExtractRoutes_EmptyConfig tests extractRoutes with empty/nil values
func TestExtractRoutes_EmptyConfig(t *testing.T) {
	// Create a minimal service for testing extractRoutes
	// Note: In production, we'd use dependency injection/interfaces for better testing
	s := &ProxyService{}
	...
	testCases := []struct {
		...
		{
			name:     "Nil apps",
			config:   map[string]interface{}{"apps": nil},
			expected: 0,
		},
		{
			name: "No http app",
				...
				"apps": map[string]interface{}{
					"http": map[string]interface{}{
						"servers": map[string]interface{}{
							"srv0": map[string]interface{}{
								"routes": []interface{}{
									map[string]interface{}{"@id": "proxy_1"},
									map[string]interface{}{"@id": "proxy_2"},
								},
							},
						},
					},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			routes := s.extractRoutes(tc.config)
			if len(routes) != tc.expected {
				t.Errorf("Expected %d routes, got %d", tc.expected, len(routes))
			}
		})
	}
}
