...
mkdir -p /etc/caddy/backup
mkdir -p /var/log/caddy

# Note: JSON configuration (caddy.json) is generated by the backend based on
# settings and proxy configurations. See the SyncService for details.
...
echo "Starting Caddy..."
/usr/bin/caddy run --config /etc/caddy/caddy.json &
CADDY_PID=$!

# Wait for Caddy to be ready (check admin API)
